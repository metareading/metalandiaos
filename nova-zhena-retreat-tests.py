...
class Т2_Програма(Основа):
    ...
    def test_15_редът_на_секциите(self):
        """КАНОН-РЕД v1.8 (MET-812 · наратив-реордер КОЙ → ЗАЩО → КАК → КАКВО · 17.09) · доказателство защо:
        • промптът на MET-812 (A1): КОЙ (hero · лийд · за кого) → ЗАЩО (лентата + метасферата — и двете ВЪТРЕ в #hero, не секции) →
          КАК (пътечката · петте дни · ритмомер „признат от UNESCO“ · Водач-био) → КАКВО (цени · багаж · ЧЗВ);
        • думите на Митрандир 16.09 („ЗА Ф5.1“ · хедърът на файла): (а) атентиометърът ПОД Его → monster → scale съседни ·
          (в) Ден-ритмомер ∪ ядрото „5 дни“ = един контрол → day → sched съседни (структурно остават 2 секции · 11-те са инвариант · test_84);
        • Водач-био е ПОСЛЕДЕН в КАК по промпта → vodach след scale; колелото остава СЛЕД сърцето и атентиометъра, защото текстът му е
          заключен („остави живата“ · решение 13) и предполага двете преди него („сърцето бие по нивото на атентиометъра“) → (г) „колелото нагоре“ → ЗА ДУМА;
        • децата на MET-633 (659 · 660 · 662 · 663 · 667) носят 8-стъпковия ред на nova-zhena-v2.html, не тези 11 секции (get_issue 17.09).
        Промени спрямо v1.7: sched 8 → 4 · scale ↔ vodach (scale под monster). test_73 (scale над wheel) остава валиден."""
        ids = секции(self.src)
        очаквано = ['hero', 'path', 'day', 'sched', 'monster', 'scale', 'vodach', 'wheel', 'bring', 'price', 'form']
        self.assertEqual(ids, очаквано, 'канон-редът на 11-те секции (v1.8 · КОЙ → ЗАЩО → КАК → КАКВО)')
        self.assertEqual(ids[-3:], ['bring', 'price', 'form'], 'КАКВО е последно: багаж · цени · ЧЗВ/форма')
        ...
        self.assertLess(ids.index('vodach'), ids.index('wheel'), 'колелото (синтезът) идва след сърцето')
...
class Т4_Цени(Основа):
    ...
    def test_31_звездичката_и_курсът(self):
        ...
        self.assertAlmostEqual(self.курс(), 121.98, delta=0.01, msg='курсът 17.09 (v1.8 · заземен същия ден): 121,98 €/g')
...
class Т9_КартаНаСвета(Основа):
    ...
    def test_90_контролът_е_в_hero_под_фигурата_не_секция(self):
        ...
        self.assertEqual(секции(self.src), ['hero', 'path', 'day', 'sched', 'monster', 'scale', 'vodach', 'wheel', 'bring', 'price', 'form'], 'редът на 11-те секции = канон-редът v1.8 (test_15 · MET-812)')
...
class Т10_ФиналУау(Основа):
    ...
    def test_100_алхимичната_лента_V2_под_hero_преди_метасферата(self):
        ...
        self.съдържа(h, 'Алхимията не е желязо в злато, а страдание в съзнание. Затова всичко Мета при нас е злато, всичко Метта — светлина. <em>В коя от 12-те сфери ще се разширявате?</em>', 'решение 1 · дословно')
    ...
    def test_106_курсът_17_09_и_грамовете(self):
        self.assertIn("КУРС={дата:'17.09.2026',usdOz:4355.80,usdEur:0.871,гр:31.1034768}", self.sc[3], 'курсът заземен 17.09 (api.gold-api.com + frankfurter.dev)')
        p = блок_по_id(self.src, 'price'); self.съдържа(p, 'Курс: 121,98 € за грам злато · 17.09.2026', 'курсът с датата 17.09')
        for g in ('13,09', '10,83', '6,47', '5,00'):
            self.съдържа(p, f'{g}<small> g</small>', f'{g} g статично')
        self.assertNotIn('13,24', p, 'старият грам-брой от 16.09 го няма'); self.assertIn('2026-09-17 (api.gold-api.com XAU 4355,80', self.src[:self.src.index('</head>')], 'хедърът носи заземяването на курса')
