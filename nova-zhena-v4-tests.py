...
class Т0_Основа(Основа):
    ...
    def test_01_хедърът_носи_текущата_версия(self):
        ...
        # то живее в ИСТОРИЯТА ("предишен ред: v4.0 · ...") и си остава там завинаги → мъртъв гард, зелен без значение
        # какво пише текущият ред. Сега гледаме САМО сегмента ПРЕДИ първото "предишен ред:" (= текущият запис).
        head = '\n'.join(self.src.splitlines()[:60])
        текущ = head.split('предишен ред:', 1)[0]
        self.assertIsNotNone(re.search(r'v4\.\d+(?:\.\d+)?', текущ), 'текущият ред трябва да носи версия v4.X')
        self.assertIsNotNone(re.search(r'v4\.\d+(?:\.\d+)?[^\n]*2026-09-\d\d[^\n]*\d\d:\d\d', текущ),
                              'текущият ред (преди първото "предишен ред:") трябва да носи v4.X + дата/час на ПОСЛЕДНАТА промяна, не историята')
