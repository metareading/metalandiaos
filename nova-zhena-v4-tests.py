...
Зелени от Т0: шевът (1-ви <script>) байт-идентичен с базата 167dd08 · деплой-копието = източник ·
  хедърът носи версия+дата+час на ПЪРВИЯ си ред (този, който започва с „НОВА ЖЕНА“ · историята под „предишен ред:“ не се брои — test_01) ·
  `#fig-heart{overflow:visible}` живее ВЪТРЕ в `@media (min-width:900px)` (test_08) · 0 „оферта" · 0 цени · нула външен <script src>.
...
ХЕДЪР_ИД = 'НОВА ЖЕНА'   # test_01 · идентификаторът, с който започва ТЕКУЩИЯТ (първи) ред на хедъра
...
class Т0_Основа(Основа):
    ...
    def test_01_хедърът_носи_текущата_версия(self):
        # MED-1 (ревю 14.09 · линеар-коментар 79cda2d9): старият тест търсеше буквалното 'v4.0' в първите 60 реда —
        # то живее в ИСТОРИЯТА ("предишен ред: v4.0 · ...") и си остава там завинаги → мъртъв гард.
        # LOW-1 (делта-гейт 14.09): и сегментният срез (head.split('предишен ред:')) е чуплив — доказани false-negative:
        #   (Д) счупен текущ ред + версия/дата в разказа ПОД него → сегментът пак съдържа v4.X+дата → зелено;
        #   (Е) маркерът преименуван („предишни редове:“) → срезът поглъща следващия исторически запис → зелено.
        # Затова: regex-ът гледа ТОЧНО ЕДИН ред — този, който започва с идентификатора „НОВА ЖЕНА“ — плюс
        # assertIn за самия маркер, за да падне тестът, ако историята/маркерът изчезне или се преименува.
        head = '\n'.join(self.src.splitlines()[:60])
        заглавни = [l for l in head.splitlines() if l.lstrip().startswith(ХЕДЪР_ИД)]
        self.assertEqual(len(заглавни), 1,
                         f'хедърът трябва да носи ТОЧНО един ред, започващ с „{ХЕДЪР_ИД}“ (текущият запис) · намерени: {len(заглавни)}')
        текущ = заглавни[0]
        self.assertIsNotNone(re.search(r'v4\.\d+(?:\.\d+)?', текущ), 'текущият (първи) ред на хедъра трябва да носи версия v4.X')
        self.assertIsNotNone(re.search(r'v4\.\d+(?:\.\d+)?[^\n]*2026-09-\d\d[^\n]*\d\d:\d\d', текущ),
                              'ПЪРВИЯТ ред на хедъра трябва да носи v4.X + дата/час на ПОСЛЕДНАТА промяна (историята не се брои)')
        self.assertIn('предишен ред:', head,
                      'маркерът „предишен ред:“ трябва да стои в хедъра — историята се append-ва, не се изтрива/преименува')
    ...
    def test_08_overflow_visible_на_сърцето_е_само_на_десктоп(self):
        # LOW-2 (делта-гейт 14.09): гард срещу безшумен регрес на MED-2 от предишния цикъл —
        # `#fig-heart{overflow:visible}` трябва да живее ВЪТРЕ в `@media (min-width:900px)`,
        # иначе мобилният bleed (.mfig{overflow:hidden}) се чупи. Мерим с баланс на скоби.
        css = re.sub(r'/\*.*?\*/', '', '\n'.join(re.findall(r'<style>(.*?)</style>', без_коментари(self.src), re.S)), flags=re.S)
        правило = re.compile(r'#fig-heart\s*\{[^}]*overflow\s*:\s*visible')
        удари = [m.start() for m in правило.finditer(css)]
        self.assertEqual(len(удари), 1, f'точно едно правило #fig-heart{{overflow:visible}} се очаква · намерени: {len(удари)}')
        медия = re.search(r'@media\s*\(\s*min-width\s*:\s*900px\s*\)\s*\{', css)
        self.assertIsNotNone(медия, 'липсва @media (min-width:900px) блок')
        дълбочина, край = 1, None
        for i in range(медия.end(), len(css)):
            if css[i] == '{':
                дълбочина += 1
            elif css[i] == '}':
                дълбочина -= 1
                if дълбочина == 0:
                    край = i
                    break
        self.assertIsNotNone(край, '@media (min-width:900px) блокът не се затваря')
        self.assertTrue(медия.end() < удари[0] < край,
                        'overflow:visible на #fig-heart трябва да е ВЪТРЕ в @media (min-width:900px) — извън него реже мобилния bleed')
