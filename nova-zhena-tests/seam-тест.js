...
window.НЖтест = (() => {
  ...
  const попълни = d => { set('name', d.name); set('email', d.email); set('phone', d.phone || ''); radio(d.mode || 'online'); set('full', d.full !== false); if (d.question) set('question', d.question); if (d.variant) НЖшев.вариант(d.variant); if ('contact' in d) set('contact', !!d.contact); };
  return {
